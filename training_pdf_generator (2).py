...
def render_token_list(tokens, styles):
    ...
    while i < len(tokens):
        ...
        # ── SESSION_NOTES ──────────────────────────────────────────────────────
            ...
            story.append(Spacer(1, 6))
            # If inner content contains nested tags (e.g. BOX_CALLOUT inside
            # SESSION_NOTES), render recursively rather than as flat text.
            # Flat text rendering drops tag content as raw markup strings.
            raw_inner = tokens_to_text(inner)
            if '[' in raw_inner:
                # Wrap in gray container and render children recursively
                child_flowables = render_token_list(inner, styles)
                if child_flowables:
                    rows = [[item] for item in child_flowables]
                    sn_t = Table(rows, colWidths=[CONTENT_W])
                    sn_t.setStyle(TableStyle([
                        ('BACKGROUND',    (0, 0), (-1, -1), GRAY_BG),
                        ('BOX',           (0, 0), (-1, -1), 0.5, GRAY_LIGHT),
                        ('LEFTPADDING',   (0, 0), (-1, -1), 12),
                        ('RIGHTPADDING',  (0, 0), (-1, -1), 12),
                ...
                story.append(session_notes_block(raw_inner, styles))
            story.append(Spacer(1, 10))
