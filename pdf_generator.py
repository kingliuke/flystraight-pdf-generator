...
def apply_inline(text):
    """Convert **bold** and *italic* to ReportLab XML. Safe for table cells."""
...
def cover_block(inner_markup, styles):
    ...
    rows = []
    rows.append([Paragraph('Prepared Exclusively For:', styles['cover_label'])])
    if program_name:
...
def generate_fuel_protocol_pdf(markup_content: str, client_name: str) -> io.BytesIO:
    ...
    story  = parse_markup(markup_content, styles)
