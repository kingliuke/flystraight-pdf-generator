...
def callout_box(text, bg, border, styles):
    ...
    # Flat single-column table: background + border applied directly so ReportLab
    # can split rows across pages. A nested Table([[inner]]) single-cell wrapper
    # cannot be split and crashes with "too large on page" for tall content.
...
def calc_block(text, styles):
    ...
    # Flat table with bg + border applied directly — splittable across pages.
    t = Table(rows, colWidths=[CONTENT_W])
    t.setStyle(TableStyle([
