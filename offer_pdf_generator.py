...
Changes from v1:
  - Added H4 style and render handler (component name headers)
  - Added bullet/dash strip in callout_box line loop (safety net)
"""
...
def callout_box(text, bg, border, styles, is_quote=False):
    ...
    for line in lines:
        ...
        # produces them inside a box despite the prose mandate. Prevents
        # them rendering as broken literal text with no indentation.
        if line.startswith(('- ', '• ', '* ')):
            line = line[2:]
...
def styled_table_options(rows):
    ...
    Expected columns: Option | Duration | Weight Loss | Projected Weight | Investment
    ...
    # Column widths
    col_widths = [
        1.4 * inch,   # Option name
        0.9 * inch,   # Duration
        1.1 * inch,   # Weight loss range
        1.2 * inch,   # Projected weight
        1.9 * inch,   # Investment (full/split)
    ]
    ...
    for i, row in enumerate(rows):
        ...
        if i == 0:
            ...
            # Check if this is recommended option (contains ★)
            option_name = str(row[0])
            is_recommended = '★' in option_name

            # Style first column (option name) as bold if recommended
            style_first = cell_bold if is_recommended else cell_style

            styled_row = [
                Paragraph(apply_inline(option_name), style_first),
                Paragraph(apply_inline(str(row[1])), cell_style),  # Duration
                Paragraph(apply_inline(str(row[2])), cell_bold),   # Weight loss (bold)
                Paragraph(apply_inline(str(row[3])), cell_style),  # Projected weight
                Paragraph(apply_inline(str(row[4])), cell_style),  # Investment
            ]
        styled_rows.append(styled_row)
    ...
    # Highlight recommended option row with light green background
    for i in range(1, len(styled_rows)):
        row_text = str(rows[i][0])
        if '★' in row_text:
            style_cmds.append(('BACKGROUND', (0, i), (-1, i), GREEN_LIGHT))
        else:
            # Alternating backgrounds for non-recommended
            bg = WHITE if i % 2 == 1 else LIGHT_GRAY
            style_cmds.append(('BACKGROUND', (0, i), (-1, i), bg))
...
def parse_markup_to_story(markup, styles):
    ...
    for token in tokens:
        ...
        # ── Text ────────────────────────────────────────────────
            ...
            # Component name header — added in v2
            story.append(Spacer(1, 4))
        ...
        # ── Tables ───────────────────────────────────────────────
            ...
            # Fitness offer options comparison table
            rows = parse_pipe_table(inner)
            if rows:
                story.append(Spacer(1, 6))
