...
Changes in v8:
  - Added TESTIMONIALS tag: renders 7 hardcoded testimonial image URLs
    as a 2-column grid. URLs are set in TESTIMONIAL_IMAGES at the top of
    the file — replace placeholders with real GHL image URLs.
    Format: [TESTIMONIALS][/TESTIMONIALS]  (inner content ignored)
  - Fixed styled_table_options to handle both 4-column tables (chat nudge
    PDF: Option | Duration | Accountability | Results) and the existing
    5-column tables (call PDF: adds Investment column). Column count is
    detected from the header row and the correct widths applied.
  - All v7 features retained (COVER_CLIENT, COMPONENT, BOX_INVESTMENT,
    footer, H3/H4, bullet strip)
"""
...
# Replace each placeholder with the real GHL-hosted image URL.
# These render as a 2-column grid via the [TESTIMONIALS][/TESTIMONIALS] tag.
TESTIMONIAL_IMAGES = [
    'TESTIMONIAL_URL_1',   # Replace with real URL
    'TESTIMONIAL_URL_2',   # Replace with real URL
    'TESTIMONIAL_URL_3',   # Replace with real URL
    'TESTIMONIAL_URL_4',   # Replace with real URL
    'TESTIMONIAL_URL_5',   # Replace with real URL
    'TESTIMONIAL_URL_6',   # Replace with real URL
    'TESTIMONIAL_URL_7',   # Replace with real URL
]
...
def styled_table_options(rows):
    ...
    Supports two layouts detected from the header row column count:

    4-column (chat nudge PDF — no investment column):
      Option | Duration | Accountability | Results

    5-column (call PDF — with investment column):
      Option | Duration | Weight Loss | Projected Weight | Investment
    ...
    # Detect layout from actual column count in header row
    num_cols = len(rows[0])

    if num_cols <= 4:
        # ── 4-column layout: chat nudge PDF ──────────────────────────────
        col_widths = [
            1.6 * inch,   # Option
            1.0 * inch,   # Duration
            1.5 * inch,   # Accountability
            2.4 * inch,   # Results
        ]
    else:
        # ── 5-column layout: call PDF ─────────────────────────────────────
        col_widths = [
            1.4 * inch,   # Option
            0.9 * inch,   # Duration
            1.1 * inch,   # Weight Loss
            1.2 * inch,   # Projected Weight
            1.9 * inch,   # Investment
        ]
    ...
    styled_rows = []
    for i, row in enumerate(rows):
        # Pad or trim to the detected column count
        ...
        if i == 0:
            ...
            styled_row = [style_first if j == 0 else cell_style for j in range(num_cols)]
            styled_row = [
                Paragraph(apply_inline(str(row[j])),
                          cell_bold if (j == 0 and is_recommended) or j == 2
                          else cell_style)
...
# ─── HELPER: testimonials grid (TESTIMONIALS) ────────────────────────────────
def testimonials_grid(styles):
    """
    Renders [TESTIMONIALS][/TESTIMONIALS] — a 2-column grid of the 7
    hardcoded testimonial images defined in TESTIMONIAL_IMAGES.

    Each image is fetched from its URL, loaded into a BytesIO buffer, and
    rendered as a ReportLab Image at a fixed width of 3.1 inches with
    proportional height. Images that fail to load (bad URL, network error)
    are silently replaced with a light-gray placeholder rectangle so the
    document never crashes.

    Layout: 2 columns, left-aligned, 0.15-inch gutter between columns.
    If the image count is odd the last row has one image in the left cell
    and an empty right cell.
    """
    IMG_WIDTH   = 3.1 * inch
    GUTTER      = 0.15 * inch
    PLACEHOLDER = colors.HexColor('#EEEEEE')

    def load_image(url):
        """Fetch URL → BytesIO → ReportLab Image, or return None on failure."""
        try:
            with urllib.request.urlopen(url, timeout=10) as resp:
                data = resp.read()
            buf = io.BytesIO(data)
            img = Image(buf, width=IMG_WIDTH)
            img.hAlign = 'LEFT'
            return img
        except Exception:
            return None
    ...
    def placeholder_cell():
        """Gray box used when an image cannot be loaded."""
        box = Table(
            [[Paragraph('', ParagraphStyle('ph', fontSize=1))]],
            colWidths=[IMG_WIDTH],
            rowHeights=[IMG_WIDTH * 0.6],
        )
    ...
    # Build cell list — one entry per testimonial slot
    cells = []
    for url in TESTIMONIAL_IMAGES:
        img = load_image(url) if (url and not url.startswith('TESTIMONIAL_URL')) else None
        cells.append(img if img else placeholder_cell())
    ...
    col_w = IMG_WIDTH + GUTTER
    t = Table(rows, colWidths=[col_w, col_w])
    t.setStyle(TableStyle([
        ('LEFTPADDING',   (0, 0), (-1, -1), 0),
        ('RIGHTPADDING',  (0, 0), (-1, -1), int(GUTTER)),
        ('TOPPADDING',    (0, 0), (-1, -1), 6),
        ('BOTTOMPADDING', (0, 0), (-1, -1), 6),
        ('VALIGN',        (0, 0), (-1, -1), 'TOP'),
    ]))
...
def parse_markup_to_story(markup, styles):
    ...
    for token in tokens:
        ...
        # ── Special ──────────────────────────────────────────────
            ...
            # Testimonial image grid — added v8
            # Inner content is ignored; images come from TESTIMONIAL_IMAGES
            story.append(Spacer(1, 8))
            story.append(testimonials_grid(styles))
            story.append(Spacer(1, 8))
